	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm[!]Masukan Opsinya Cukk...')
				continue
		else:
			if len(a) == 0:
				tampil('\rm[!]Masukan yang benar')
				continue
			else:
				break
	return a
...
def menu():
	tampil('''\rh
	 	...
	 	╚═╝  ╚═╝ ╚══▀▀═╝ ╚═╝╚══════╝
 ========================================================
 =	   BRUTE FORCE FACEBOOK (Ngehek FB:V)   	=
 = 							=
 =  Author = Syaid 					=
 =  contact = syaidagil21@gmail.com 			=
